...
def generate_watchbill():
    ...
    try:
        ...
        watchbill_data = {}
        assigned_sailors = {}  # Change to dictionary to store assignments per time slot

        for station in watchstations:
            watchbill_data[station] = {}
            assigned_sailors[station] = {}  # Initialize for each station
            ...
            for start, end in watchtimes:
                ...
                for last_name, qualifications in sailors:
                    if qualifications:
                        ...
                        is_qualified = any(station == qual or (station.startswith(qual) and station[len(qual):].isdigit()) for qual in sailor_quals)

                        if is_qualified:
                            if station == "OOD":
                                if last_name not in [assigned_sailors[st].get(f"{start} - {end}") for st in assigned_sailors]:
                                    qualified_sailors.append(last_name)

                            elif last_name not in [assigned_sailors[st].get(f"{start} - {end}") for st in assigned_sailors if st != "Internal Rover"]:
                ...
                if qualified_sailors:
                    ...
                    assigned_sailors[station][f"{start} - {end}"] = chosen_sailor # Store the assignment

                else:
                    watchbill_data[station][f"{start} - {end}"] = "UNQUALIFIED/ASSIGN MANUALLY"
    ...
    def display_watchbill(selected_date, watchbill_data, watchstations, watchtimes):
        ...
        def display_watchbill(selected_date, watchbill_data, watchstations, watchtimes):
            ...
            def on_double_click(event):
                ...
                if rowid and colid and colid != '#0':  # Check for valid row and column

                    current_value = watchbill_tree.item(rowid)['values'][int(colid[1:]) - 1]
                    if current_value == "UNQUALIFIED/ASSIGN MANUALLY":  # Check against placeholder
